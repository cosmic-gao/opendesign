...
### 2.1 设计理念 - Headless UI 思想

**核心原则**：采用 Headless UI 思想，只提供**布局行为和状态逻辑**，不提供具体样式。

| 原则 | 说明 |
|------|------|
| 只管逻辑不管样式 | 组件内部处理布局计算、状态管理，但不渲染任何 CSS |
| 完全可自定义 | 通过 className、style 完全控制视觉效果 |
| 状态透出 | 所有布局状态通过 hooks 暴露，便于外部控制 |
| 默认值友好 | 提供合理的布局默认值，但不强制任何视觉风格 |
