...
export interface Emitter<E extends object> {
  ...
  once<K extends EventType<E>>(type: K, handler: Handler<E[K]>): () => void;
  ...
  emit<K extends EventType<E>>(type: K, event: E[K]): void;
  emit<K extends EventType<E>>(type: undefined extends E[K] ? K : never): void;
...
export class Signal<E extends object = Record<string, unknown>>
  ...
  public on(type: EventType<E> | "*", handler: Callback<E>): () => void {
  ...
  public once(type: EventType<E> | "*", handler: Callback<E>): () => void {
    ...
    }) as Listener<E[EventType<E>], Callback<E>>;
  ...
  public off(type: EventType<E> | "*", handler?: Callback<E>): void {
    const handlers = this.all.get(type);
    if (!handlers) return;
    ...
    const index = handlers.findIndex((h) =>
      h === handler ||
      (h as Listener<E[EventType<E>], Callback<E>>).source === handler
    );
   ...
   * ```
   */
  public emit<K extends EventType<E>>(type: K, event: E[K]): void;
